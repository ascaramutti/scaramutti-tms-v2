 ...
 * campo (name O ruc) — un AND de ORs, no un OR de la cadena completa (via
 * {@link com.scaramutti.tms.shared.util.MultiWordSearch}, compartido con
 * ProductRepository). Es una decision explicita del dueno para evitar la deuda
 * de busqueda multi-palabra que quedo pendiente en cotizaciones.
 */
@ApplicationScoped
public class SupplierRepository implements PanacheRepositoryBase<Supplier, Integer> {
    ...
    private String buildWhere(String q, Boolean isActive, Map<String, Object> params) {
        ...
        if (q != null) {
            conditions.addAll(MultiWordSearch.conditions(q, List.of("name", "ruc"), "qTok", params));
        }
        if (isActive != null) {
