 ...
 *
 * <p>Orden del flow: cargar el viaje, rechazar los dos estados inmutables, verificar la version,
...
public class UpdateServiceService {
    ...
    public ServiceDetailResponse updateService(UpdateServiceCommand command) {
        ...
        if (changes.isEmpty()) {
            return readDetail(service.id);
        }

        serviceRowLock.runTranslatingLockConflicts(() -> {
            ...
            // Este volcado NO es defensivo: el gancho que mueve la version corre AL VOLCAR, y
            // ningun volcado automatico lo cubre. La consulta que arma la bitacora podria
            // dispararlo, pero para cuando corre ya no queda nada encolado: las filas de bitacora
            // y de auditoria las genera la base (id IDENTITY), asi que su INSERT se ejecuta en el
            // acto al persistirlas, y lo unico que sigue pendiente es el UPDATE del servicio.
            // Sin esta linea el detalle se arma con la version VIEJA: la respuesta sale con un
            // ETag que la base ya no tiene, y el proximo If-Match del cliente responde 412 para
            // siempre hasta que vuelva a leer el detalle. Verificado quitandola.
    ...
    /**
     * El detalle que devuelve la respuesta, con el conflicto de lock traducido igual que las
     * escrituras: son media docena de consultas mas (bitacora, autores, catalogos) y corren con el
     * mismo tope vigente, asi que tambien pueden chocar — por ejemplo contra una migracion que
     * tome una de esas tablas durante un deploy. Sin traducirlas, ese choque sale 500 en vez del
     * 409 transitorio que declara el contrato.
     */
    private ServiceDetailResponse readDetail(long serviceId) {
        return serviceRowLock.runTranslatingLockConflicts(
            () -> getServiceService.getService(serviceId), serviceId);
    }
     ...
     *
     * <p>Asignar el resto igual seria inofensivo hoy —los valores serian identicos a los
     * guardados—, pero dejaria la correccion sostenida por una coincidencia: que la comparacion
     * del diff y la que Hibernate usa para decidir si la fila esta sucia den siempre el mismo
     * resultado, siendo dos mecanismos de capas distintas que nadie ata. Donde esa coincidencia
     * ya esta tensa es en las fechas reales: el diff compara INSTANTES (a proposito, para no
     * depender del huso con el que las devuelva el driver) y una asignacion directa escribe el
     * objeto con su huso. Si esos dos criterios se separan, la fila se ensucia sin ningun cambio
     * contado: se mueve la version y NO se escribe auditoria ni bitacora, que es dato sin rastro
     * en el modulo cuya razon de ser es la rendicion de cuentas.
     *
     * <p>Con esta forma la regla es estructural y no hay nada que recordar: lo que el diff no
     * conto, la columna no lo toca. Los tres textos libres ademas lo NECESITAN, porque un valor
     * guardado que difiere solo en espacios (dato tipico del sistema anterior) no es un cambio y
     * reescribirlo limpiaria la base colado en la edicion de otro campo.
     */
    private void applyChanges(Service service, UpdateServiceCommand command,
            ...
            Integer userId) {
        applyIfChanged(changes, "tentativeDate", () -> service.tentativeDate = command.tentativeDate());
        applyIfChanged(changes, "origin", () -> service.origin = command.origin());
        applyIfChanged(changes, "destination", () -> service.destination = command.destination());
        applyIfChanged(changes, "weight", () -> service.weight = command.weightKg());
        applyIfChanged(changes, "length", () -> service.length = command.lengthM());
        applyIfChanged(changes, "width", () -> service.width = command.widthM());
        applyIfChanged(changes, "height", () -> service.height = command.heightM());
        applyIfChanged(changes, "price", () -> service.price = command.price());
        applyIfChanged(changes, "currency", () -> service.currencyId = command.currencyId());
        applyIfChanged(changes, "observations", () -> service.observations = command.observations());
        applyIfChanged(changes, "startDateTime", () -> service.startDateTime = newStart);
        applyIfChanged(changes, "endDateTime", () -> service.endDateTime = newEnd);
        // Este NO sale del diff: no es un campo del viaje, es quien lo tocó.
        service.updatedBy = userId;
        // La version NO se asigna aca: la mueve el @PreUpdate de la entity, que es una sola
        // fuente para todos los endpoints que van a escribir sobre esta misma fila. Solo se
        // llega a este punto con algun campo distinto, asi que la fila queda sucia y el gancho
        // dispara seguro.
    }

    /** Escribe el campo solo si el diff lo conto como cambio. */
    private void applyIfChanged(List<FieldChange> changes, String fieldName, Runnable assignment) {
        if (changes.stream().anyMatch(change -> change.name().equals(fieldName))) {
            assignment.run();
        }
    }
