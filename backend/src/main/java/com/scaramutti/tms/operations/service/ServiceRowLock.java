...
public class ServiceRowLock {
    ...
    /** Version sin resultado del anterior, para los bloques que solo escriben. */
    public void runTranslatingLockConflicts(Runnable action, long serviceId) {
        runTranslatingLockConflicts(() -> {
            action.run();
            return null;
        }, serviceId);
    ...
    int requireUsableLockTimeout() {
        if (lockTimeoutSeconds <= 0 || lockTimeoutSeconds >= poolAcquisitionTimeout.toSeconds()) {
            throw new IllegalStateException(
                "app.operations.edit-lock-timeout-seconds tiene que estar entre 1 y "
                    + (poolAcquisitionTimeout.toSeconds() - 1) + " segundos (0 desactiva el tope en "
