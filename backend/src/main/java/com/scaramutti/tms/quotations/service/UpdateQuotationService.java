...
public class UpdateQuotationService {
    ...
    public QuotationResponse updateQuotation(Long id, String ifMatch, SaveQuotationCommand command) {
        ...
        // 3. Terminalidad: una cotizacion en estado terminal (ACCEPTED/REJECTED/EXPIRED) es
        //    inmutable — no se edita (ADR-004/006). 409 QUO-006, sin tocar la fila.
        verifyNotTerminal(quotation);

        // 4. Campos inmutables: la cotizacion pertenece a un cliente y su tipo define las reglas.
        verifyImmutableFields(command, quotation);

        // 5. Cargar dependencias + validar reglas de negocio (reuso del create).
        ...
        // 7. Actualizar la cabecera (solo campos editables; los inmutables se preservan).
        quotationServiceMapper.applyUpdate(quotation, command, userId);

        // 8. REPLACE de items: borrar standby PRIMERO (FK quotation_item_id NOT NULL), luego
    ...
    /**
     * Una cotizacion en estado terminal ({@code ACCEPTED}/{@code REJECTED}/{@code EXPIRED})
     * es inmutable: no se puede editar (ADR-004/006). Si lo esta → 409 QUO-006, sin tocar la
     * fila. {@code DRAFT}/{@code SENT} siguen editables.
     */
    private void verifyNotTerminal(Quotation quotation) {
        QuotationStatus status = QuotationStatus.valueOf(quotation.status);
        if (status.isTerminal()) {
            throw QuotationsError.TERMINAL_NOT_EDITABLE.toException(
                "No se puede editar una cotizacion en estado " + status
            );
        }
    }
    ...
    /**
     * {@code isExpired} se DERIVA del estado persistido ({@code status == EXPIRED}, ADR-005) —
     * misma fuente de verdad que el GET/LIST. Tras un PUT el estado nunca es {@code EXPIRED}
     * (editar un terminal se bloquea en el paso 3 con QUO-006), asi que en la practica devuelve
     * {@code false}; se mantiene la derivacion por consistencia y para no recomputar por fechas.
     */
